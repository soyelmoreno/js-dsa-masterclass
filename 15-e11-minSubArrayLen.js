...
// DSA course solution
function minSubArrayLen_COURSE(nums, sum) {
  let total = 0;
  let start = 0;
  let end = 0;
  let minLen = Infinity;
...
// My version plus an explanation from NeetCode video
// https://www.youtube.com/watch?v=aYqYMIqZx5s&t=1s
...
function minSubArrayLen(arr, target) {
  ...
  for (let j = 0; j < arr.length; j++) {
    ...
    sum = sum + arr[j]; // 8
    while (sum >= target) {
      minLen = Math.min(minLen, j - i + 1); // 4
      sum -= arr[i];
      i++;
    }
  }
